  ...
  return (
    <div class="absolute flex items-center h-12 pl-4 pb-1" data-tauri-drag-region>
      <div class="flex space-x-2" onMouseOver={() => setHover(true)} onMouseLeave={() => setHover(false)}>
        <button
          class={`${buttonClass} ${isUnFocused() ? 'bg-gray-400 opacity-40' : 'bg-red-400'}`}
          onClick={appWindow.close}
        >
          {hover() ? <IoClose size={12} /> : null}
        </button>
        <button
          class={`${buttonClass} ${isUnFocused() ? 'bg-gray-400 opacity-40' : 'bg-amber-400'}`}
          onClick={appWindow.minimize}
        >
          {hover() ? <IoRemove size={12} /> : null}
        </button>
        <button
          class={`${buttonClass} ${isUnFocused() ? 'bg-gray-400 opacity-40' : 'bg-green-400'}`}
