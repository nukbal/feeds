...
import { detailRoute, FeedDetailRouteType } from 'models/route';

async function fetcher({ name, feed, id, subId }: FeedDetailRouteType) {
  if (!name || !feed || !id) return null;
...
export default function FeedDetailPage() {
  const [param] = detailRoute;
  const [data] = createResource(() => param(), fetcher);

  let pageRef: HTMLDivElement | undefined;

  createEffect(() => {
    pageRef?.scrollTo({ top: 0, left: 0 });
  }, param().id)
  ...
  return (
    <FeedDetail ref={pageRef}>
      <Switch fallback={<FeedDetailEmpty />}>
